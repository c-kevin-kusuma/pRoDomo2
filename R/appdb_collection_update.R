#' Update AppDB Collection
#'
#' Updates an existing AppDB collection's name, schema, and/or sync
#' setting.
#'
...
#'
#' @examples
#' response <- appdb_collection_update(
#'   collection_id = "12345678-1234-1234-1234-123456789012",
#'   name = "A Renamed Collection",
...
appdb_collection_update <- function(
  collection_id,
  name,
  developer_token,
  instance,
  ...
  # PUT is idempotent (replacing the same collection with the same
  # content has no additional side effect), so retrying on a transient
