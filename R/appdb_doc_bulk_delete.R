#' Bulk Delete AppDB Documents
#'
#' Deletes multiple AppDB documents in a single API call.
#'
#' @param collection_id The AppDB collection ID.
#' @param appdb_document_ids A character vector of document IDs to
#'   delete.
...
#'
#' @examples
#' response <- appdb_doc_bulk_delete(
#'   collection_id = "12345678-1234-1234-1234-123456789012",
#'   appdb_document_ids = c(
...
appdb_doc_bulk_delete <- function(
  collection_id,
  appdb_document_ids,
  developer_token,
  instance
  ...
  request <- httr2::req_url_query(
    request,
    ids = paste(appdb_document_ids, collapse = ",")
  )
  ...
  # DELETE is idempotent (deleting an already-deleted document is a
